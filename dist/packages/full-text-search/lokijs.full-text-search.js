...
class FullTextSearch {
    ...
    static fromJSONObject(serialized, tokenizers) {
        ...
        for (let i = 0, fieldName; i < fieldNames.length, fieldName = fieldNames[i]; i++) {
            fts._invIdxs[fieldName] = __WEBPACK_IMPORTED_MODULE_0__inverted_index__["a" /* InvertedIndex */].fromJSONObject(db[fieldName], tokenizers[fieldName]);
        }
        return fts;
    }
    setDirty() {
...
class WildcardSearch {
     ...
     * @private
     */
    _recursive(index, idx = 0, term = "", escaped = false) {
        if (index === null) {
            return;
        }
        ...
        if (!escaped && this._wildcard[idx] === "\\") {
            ...
            this._recursive(__WEBPACK_IMPORTED_MODULE_1__inverted_index__["a" /* InvertedIndex */].getTermIndex(this._wildcard[idx], index), idx + 1, term + this._wildcard[idx]);
        }
    }
}
