...
class Collection extends __WEBPACK_IMPORTED_MODULE_0__event_emitter__["a" /* LokiEventEmitter */] {
    ...
    checkIndex(field, options = { repair: false }) {
        ...
        if (len === 1) {
            ...
            if (options.randomSampling) {
                ...
                if (!__WEBPACK_IMPORTED_MODULE_2__result_set__["a" /* LokiOps */].$lte(this._data[biv[0]][field], this._data[biv[1]][field])) {
                    valid = false;
                }
                if (!__WEBPACK_IMPORTED_MODULE_2__result_set__["a" /* LokiOps */].$lte(this._data[biv[len - 2]][field], this._data[biv[len - 1]][field])) {
                    valid = false;
                }
                ...
                if (valid) {
                    ...
                    for (let idx = 0; idx < iter; idx++) {
                        ...
                        if (!__WEBPACK_IMPORTED_MODULE_2__result_set__["a" /* LokiOps */].$lte(this._data[biv[pos]][field], this._data[biv[pos + 1]][field])) {
                            valid = false;
                            break;
                        }
                    }
                ...
                for (let idx = 0; idx < len - 1; idx++) {
                    if (!__WEBPACK_IMPORTED_MODULE_2__result_set__["a" /* LokiOps */].$lte(this._data[biv[idx]][field], this._data[biv[idx + 1]][field])) {
                        valid = false;
                        break;
                    }
                }
    ...
    insertOne(doc, bulkInsert = false) {
        ...
        const obj = this._defineNestedProperties(this.cloneObjects ? Object(__WEBPACK_IMPORTED_MODULE_5__clone__["a" /* clone */])(doc, this.cloneMethod) : doc);
        if (!this.disableMeta && obj.meta === undefined) {
            obj.meta = {
                version: 0,
                revision: 0,
                created: 0
    ...
    update(doc) {
        ...
        try {
            ...
            // if configured to clone, do so now... otherwise just use same obj reference
            let newInternal = this._defineNestedProperties(this.cloneObjects || !this.disableDeltaChangesApi ? Object(__WEBPACK_IMPORTED_MODULE_5__clone__["a" /* clone */])(doc, this.cloneMethod) : doc);
    ...
    chain(transform, parameters) {
        const rs = new __WEBPACK_IMPORTED_MODULE_2__result_set__["b" /* ResultSet */](this);
    ...
    startTransaction() {
        if (this.transactional) {
            this._cached = {
                index: this.idIndex,
                data: Object(__WEBPACK_IMPORTED_MODULE_5__clone__["a" /* clone */])(this._data, this.cloneMethod),
    ...
    eqJoin(joinData, leftJoinProp, rightJoinProp, mapFun, dataOptions) {
        return new __WEBPACK_IMPORTED_MODULE_2__result_set__["b" /* ResultSet */](this).eqJoin(joinData, leftJoinProp, rightJoinProp, mapFun, dataOptions);
    }
    /* ------ STAGING API -------- */
    /**
...
/* harmony export (immutable) */ __webpack_exports__["a"] = LokiOps;
...
class ResultSet {
    ...
    simplesort(propname, options = { desc: false }) {
        ...
        if (!options.disableIndexIntersect && this._collection.binaryIndices.hasOwnProperty(propname)
            ...
            if (eff <= targetEff || options.forceIndexIntersect) {
                ...
                this._filteredRows = this._collection.binaryIndices[propname].values.filter((n) => {
                    return io[n];
                });
                if (options.desc) {
        ...
        const data = this._collection._data;
        const wrappedComparer = (a, b) => {
            return Object(__WEBPACK_IMPORTED_MODULE_2__helper__["d" /* sortHelper */])(data[a][propname], data[b][propname], options.desc);
        };
        this._filteredRows.sort(wrappedComparer);
        return this;
    }
    ...
    _compoundeval(properties, obj1, obj2) {
        for (let i = 0, len = properties.length; i < len; i++) {
            const prop = properties[i];
            const field = prop[0];
            const res = Object(__WEBPACK_IMPORTED_MODULE_2__helper__["d" /* sortHelper */])(obj1[field], obj2[field], prop[1]);
            if (res !== 0) {
                return res;
            }
        }
    ...
    data(options = {}) {
        ...
        if (!this._filterInitialized) {
            if (this._filteredRows.length === 0) {
                ...
                if (this._collection.cloneObjects || forceClones) {
                    ...
                    for (let i = 0; i < data.length; i++) {
                        obj = this._collection._defineNestedProperties(Object(__WEBPACK_IMPORTED_MODULE_1__clone__["a" /* clone */])(data[i], method));
                        if (removeMeta) {
                            delete obj.$loki;
                            delete obj.meta;
                        }
        ...
        if (this._collection.cloneObjects || forceClones) {
            ...
            for (let i = 0; i < fr.length; i++) {
                obj = this._collection._defineNestedProperties(Object(__WEBPACK_IMPORTED_MODULE_1__clone__["a" /* clone */])(data[fr[i]], method));
                if (removeMeta) {
                    delete obj.$loki;
                    delete obj.meta;
                }
...
/* harmony export (immutable) */ __webpack_exports__["b"] = ResultSet;
...
class DynamicView extends __WEBPACK_IMPORTED_MODULE_0__event_emitter__["a" /* LokiEventEmitter */] {
    ...
    constructor(collection, name, options = {}) {
        ...
        this._resultSet = new __WEBPACK_IMPORTED_MODULE_1__result_set__["b" /* ResultSet */](collection);
        this._resultData = [];
    ...
    _rematerialize({ removeWhereFilters = false }) {
        ...
        this._resultSet = new __WEBPACK_IMPORTED_MODULE_1__result_set__["b" /* ResultSet */](this._collection);
    ...
    _evaluateDocument(objIndex, isNew) {
        ...
        const evalResultSet = new __WEBPACK_IMPORTED_MODULE_1__result_set__["b" /* ResultSet */](this._collection);
